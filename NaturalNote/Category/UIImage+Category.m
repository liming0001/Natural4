...
#define MAX_UPLOAD_IMG_WIDTH ScreenWidth*2
#define MAX_UPLOAD_IMG_HEIGHT ScreenHeight*2
...
//压缩要上传的图片
+ (UIImage *)imageWithImageUpload:(UIImage *)image
{
    ...
    if(![UIImage isLongImage:theImage]){
        theImage = [UIImage imageWithImageFileSize:theImage];
    }
...
//压缩要上传的图片
+ (NSData *)dataWithImageUpload:(UIImage *)image
{
    UIImage *theImage = image;
    NSData *theData = nil;
...
//按图片大小压缩图片尺寸
+ (UIImage *)imageWithImageFileSize:(UIImage *)image
{
    ...
    {
        if(newImage.size.width > MAX_UPLOAD_IMG_WIDTH && newImage.size.width>newImage.size.height)
        {
            CGSize size = CGSizeMake(MAX_UPLOAD_IMG_WIDTH, MAX_UPLOAD_IMG_WIDTH*newImage.size.height/newImage.size.width);
            newImage = [UIImage compressionWithImage:newImage toSize:size];
            
        }
        else if(newImage.size.height>MAX_UPLOAD_IMG_HEIGHT && newImage.size.height>newImage.size.width)
        {
            CGSize size = CGSizeMake(MAX_UPLOAD_IMG_HEIGHT*newImage.size.width/newImage.size.height, MAX_UPLOAD_IMG_HEIGHT);
            newImage = [UIImage compressionWithImage:newImage toSize:size];
        }
    }
    return newImage;
...
- (NSData *)lx_compressImageWithMaxLength:(NSInteger)maxLength {
    NSData *imageData = UIImageJPEGRepresentation(self, 0.2);
    CGFloat i = 1;
    CGSize imageSize = self.size;
    while (imageData.length > maxLength && i > 0.2) {
        CGSize newSize = CGSizeMake(imageSize.width*i, imageSize.height*i);
